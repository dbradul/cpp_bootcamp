    ...
    , m_socket(new QTcpSocket(this))
{
    ...
    connect(m_socket.data(), SIGNAL(connected()),           this, SLOT(onConnected()));
    connect(m_socket.data(), SIGNAL(bytesWritten(qint64)),  this, SLOT(onBytesWritten(qint64)));
    connect(m_socket.data(), SIGNAL(readyRead()),           this, SLOT(onReadyRead()));
    connect(m_socket.data(), SIGNAL(disconnected()),        this, SLOT(onDisconnected()));
    connect(m_socket.data(), SIGNAL(error(QAbstractSocket::SocketError)),           this, SLOT(onError(QAbstractSocket::SocketError)));
    connect(m_socket.data(), SIGNAL(stateChanged(QAbstractSocket::SocketState)),    this, SLOT(onStateChanged(QAbstractSocket::SocketState)));
}
